...
Beim Durchlaufen eines \ac{MDP} entsteht eine Folge von Zuständen, Aktionen und Belohnungen, die als Trajektorie bezeichnet wird \cite[Kap. 3.1]{sutton2018rl}:
\\
...
Analog zu den diskreten Räumen modelliert \ac{SB3} \cite{sb3} kontinuierliche Aktionsräume durch die Parametrisierung einer Wahrscheinlichkeitsdichtefunktion, standardmäßig einer Gauß\hyp{}Verteilung (Normalverteilung). Anstatt Wahrscheinlichkeiten für diskrete Werte auszugeben, liefert das Modell die Parameter der Verteilung: den Erwartungswert $\mu$ (Mean) und die Standardabweichung $\sigma$ (Standard Deviation).
Das neuronale Netz gibt dabei lediglich den $n$-dimensionalen Vektor der Erwartungswerte aus, weshalb nur $n$ Ausgangsneuronen benötigt werden. Die Standardabweichung wird als separater, zustandsunabhängiger Parameter gelernt \cite{sb3}.
...
für einen gegebenen Zustand $s$ eine Wahrscheinlichkeitsverteilung über das alle möglichen Aktionen ausgibt.
\medskip

Das Ziel der Optimierung ist die Maximierung der erwarteten kumulativen Belohnung $J(\theta)$. Da Optimierungs-Frameworks wie PyTorch standardmäßig auf Gradientenabstieg (Minimierung) ausgelegt sind, wird dies äquivalent als Minimierung von $-J(\theta)$ formuliert. 
...
$\rho_t(\theta)$    & Probability Ratio & Verhältnis zwischen aktueller und alter Policy: $\frac{\pi_\theta(a_t|s_t)}{\pi_{\text{old}}(a_t|s_t)}$ \\
...
\ac{PPO} bietet gegenüber verwandten Methoden wie \ac{TRPO} (\textit{Trust Region Policy Optimization}) eine höhere Hyperparameter\footnote{Hyperparameter sind Konfigurationsparameter des Lernalgorithmus, die vor dem Training festgelegt werden und nicht während des Trainings gelernt werden (im Gegensatz zu den Modellparametern $\theta$ und $\phi$). Beispiele sind die Lernrate, die Anzahl der Epochen $n\_epochs$ oder der Clipping\hyp{}Parameter $\epsilon$.}stabilität \cite[Abschn. 2.2]{OriginalPPO} und reduziert somit die Anfälligkeit gegenüber schlechten Hyperparametereinstellungen.
\medskip
...
Advantage\hyp{}Werte genutzt, um das \textit{Clipped Surrogate Objective} zu berechnen. 
...
In der praktischen Implementierung von \ac{PPO} werden die Parameter des Actors ($\theta$) und des Critics ($\phi$) simultan über eine solche kombinierte Funktion optimiert. Die totale Verlustfunktion ergibt sich als gewichtete Summe \cite[Glg. 9]{OriginalPPO}:
...
Die Anpassung der Netzwerkparameter $\theta$ erfolgt dabei iterativ entlang des negativen Gradienten. Die Berechnung dieser Gradienten wird in Frameworks wie PyTorch\footnote{PyTorch ist eine quelloffene Python\hyp{}Bibliothek zur Entwicklung und zum Training neuronaler Netzwerke. Sie bietet umfangreiche Funktionen für Tensoroperationen und automatische Differenzierung.}
