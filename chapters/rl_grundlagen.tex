...
Diese Arbeit konzentriert sich speziell auf sogenannte \textit{bedingte} hybride Aktionsräume. Um die Abhängigkeiten zwischen den Aktionen in solchen Räumen korrekt zu behandeln, wird im weiteren Verlauf die Methode des \textit{Gradient Maskings} eingesetzt.
...
PPO setzt auf eine Actor\hyp{}Critic\hyp{}Architektur und adressiert die Instabilität klassischer Gradientenverfahren, bei denen zu große Aktualisierungsschritte die Policy in Regionen des Parameterraums treiben können, aus denen eine Erholung kaum möglich ist.
\vspace{\baselineskip}
...
\begin{itemize}
    \item \textbf{Actor\hyp{}Loss} ($-L_t^{CLIP}$): Das \textit{Clipped Surrogate Objective} begrenzt die Änderung der Policy \cite[Abschn. 3]{OriginalPPO}. Da PPO den Vorteil $A_t$ maximieren möchte, wird dieser Term für den Gradientenabstieg negiert:
    \begin{equation}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Visualisierung des Gradientenabstiegs auf einer zweidimensionalen Verlustfunktion.}
