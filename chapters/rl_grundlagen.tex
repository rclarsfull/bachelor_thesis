...
Im Gegensatz zu wertbasierten Ansätzen, die zunächst eine Wertfunktion lernen und daraus eine Policy ableiten, optimieren \textit{Policy\hyp{}Gradient}\hyp{}Methoden die Strategie $\pi_\theta$ des Agenten unmittelbar. 
...
für einen gegebenen Zustand $s$ eine Wahrscheinlichkeitsverteilung über das Aktionsspektrum ausgibt.
\vspace{\baselineskip}
...
$\pi_\theta$      & Policy & Liefert Wahrscheinlichkeiten über Aktionen, wobei die Parameter $\theta$ trainiert werden \\
...
was für das in dieser Arbeit vorgestellte \textit{Gradient Gating} essentiell ist.

\subsection{Trainingsablauf}
Der in Algorithmus \ref{alg:ppo_training} dargestellte Ablauf verdeutlicht die Trennung zwischen Datenerhebung und Optimierung, die für das Verständnis des später implementierten \textit{Gradient Gating} essenziell ist.
...
\subsubsection*{Rechengraph und Kettenregel} Während des Forward\hyp{}Passes werden alle mathematischen Operationen auf den Tensoren in 
einem dynamischen, gerichteten Graphen aufgezeichnet, der als \textit{Rechengraph} bezeichnet wird. Dieser Graph bildet die funktionale 
