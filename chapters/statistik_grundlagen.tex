...
Die Bestimmung der Konfidenzintervalle erfolgt mittels Percentile-Bootstrap. \citet{statistical_rl_evaluation} untersuchen in Appendix A.5 verschiedene Bootstrap-Varianten (Basic, Percentile, Bias-Corrected, BCa) und zeigen, dass im Kontext von Deep Reinforcement Learning mit wenigen Runs der Percentile-Bootstrap die zuverlässigste Abdeckung (Coverage) der wahren Werte liefert, während komplexere Methoden wie BCa bei kleinen Stichproben instabiler sein können. Das Vorgehen lässt sich wie folgt zusammenfassen:
\begin{enumerate}
...
Statt einfacher Mittelwerte wird der Interquartile Mean (IQM) als robuste Lagekennzahl der episodischen Rewards genutzt. Der IQM mittelt nur über das mittlere 50\%-Quantil und dämpft damit Ausreißer deutlich \citep{statistical_rl_evaluation}. Für jede Variante wird der IQM der \emph{last\_eval\_mean}-Werte berechnet, wobei das 95\%-Konfidenzintervall aus dem Percentile-Bootstrap mit \(B=5000\) resultiert.
...
Daher wird das Verfahren der Bootstrap-Differenzen verwendet. Hierfür werden, wie oben beschrieben, Bootstrap-Verteilungen für beide Varianten erzeugt. In jeder Bootstrap-Iteration \(i\) wird die Differenz der Kennzahlen berechnet: \(\Delta_i = IQM_{A,i} - IQM_{B,i}\). Dies liefert eine empirische Verteilung der Differenzen. Das 95\%-Konfidenzintervall dieser Differenz-Verteilung ermöglicht eine präzise Aussage:
\begin{itemize}
