...
\begin{itemize}
    ...
    Diese Szenarien wurden für die Simulation abstrahiert, da das aktuelle Modell ausschließlich Vectoring (Kursänderungen) betrachtet und Höhen\hyp{} oder Geschwindigkeitsanpassungen ausgeklammert werden.
\end{itemize}
...
Die Interaktion zwischen der Reinforcement\hyp{}Learning\hyp{}Umgebung (\ac{GYM}) und Bluesky erfolgt über eine dedizierte Adapter\hyp{}Schicht. Diese übersetzt die Aktionen des Agenten in simulatorverständliche Steuerbefehle und extrahiert im Gegenzug die relevanten Zustandsinformationen für das Training.
\medskip
...
Die Implementierung folgt der Spezifikation von OpenAI \ac{GYM} \cite{OpenAI-Gym}, um die Kompatibilität mit etablierten \ac{RL}\hyp{}Bibliotheken wie \ac{SB3} \cite{sb3} sicherzustellen.
...
Eine hohe Generalisierungsfähigkeit ist allerding erforderlich, damit der Agent auch in unbekannten Situationen, die nicht Teil der Trainingsdaten waren, robuste Entscheidungen trifft. Dies ist besonders wichtig wenn der Agent später in unterschiedlichen Sectoren eingesetzt werden soll.
...
Die Berechnung zukünftiger Konflikte basiert auf dem \ac{CPA}\hyp{}Verfahren (Closest Point of Approach) \cite{CPA}, wobei die Zeit bis zum Konflikt (\emph{Time to \ac{CPA}}) als Maß für die Kritikalität dient.
...
Obgleich operative Fluglotsen auch über Geschwindigkeits\hyp{} und Höhenänderungen verfügen, würde deren Integration die Komplexität der Problemstellung erheblich steigern:
...
Ein rein kontinuierlicher Aktionsraum erwies sich für die Anforderungen als unzureichend, da er die Modellierung einer expliziten \emph{Noop}-Aktion (No Operation) erschwert, also den bewussten Verzicht auf einen Eingriff. 
...
\begin{figure}[htbp]
    ...
    \node[node] {Aktionstyp\\(diskret)}
        child {
            node[node] {Steer}
            child {
                node[leaf] {Kursänderung\\$\theta \in [-180°, +180°]$\\(kontinuierlich)}
...
\ac{PPO} gehört zur Klasse der Policy\hyp{}Gradient\hyp{}Methoden (Kapitel \ref{ch:rl_grundlagen}) und hat sich in zahlreichen Anwendungen als stabiler und robuster Standard etabliert \cite{OriginalPPO}.
\medskip
...
Für die vorliegende Arbeit ist insbesondere die funktionale Breite relevant: Laut \ac{SB3}\hyp{}Dokumentation unterstützt \ac{PPO} nativ sowohl kontinuierliche als auch diskrete Aktionsräume (u.\,a. \texttt{Box}, \texttt{Discrete}, \texttt{MultiDiscrete}, \texttt{MultiBinary}) \cite{sb3_algos_doc}.
Damit lässt sich eine einheitliche Trainingspipeline über unterschiedliche Environment\hyp{}Designs hinweg nutzen.
Zusätzlich sind im \textit{sb3\hyp{}contrib}\hyp{}Ökosystem Erweiterungen wie \textit{Recurrent\ac{PPO}} und \textit{Maskable\ac{PPO}} verfügbar, was die methodische Anschlussfähigkeit für spätere Experimente erhöht \cite{sb3_algos_doc}.
\medskip
...
Die spezifischen methodischen Anpassungen des \ac{PPO}\hyp{}Algorithmus an den in Abschnitt \ref{sc:aktionsraum} beschriebenen gemischten Aktionsraum werden im nachfolgenden Kapitel \ref{ch:ppo_bedingt} vertieft.