...
\begin{enumerate}
    ...
    \item \label{hyp:no_gating} \textbf{Architektur-Robustheit:} Der PPO-Algorithmus ohne Gradient\hyp{}Gating zeigt in der modifizierten Umgebung keine signifikante Leistungsverschlechterung gegenüber dem Standard-PPO.
    ...
    \item \label{hyp:Interferenzen} \textbf{Vermeidung negativer Interferenzen:} Die angepasste Netzarchitektur mit separaten Verarbeitungspfaden für diskrete und kontinuierliche Aktionen in Kombination mit Gradient\hyp{}Gating führt zu einer weiteren Leistungssteigerung gegenüber der Standard-Gradient\hyp{}Gating-Variante, besonders in Szenarien mit einem hohen Anteil Noop\hyp{}Aktionen.
    \item \label{hyp:Netzarchitektur} \textbf{Netzarchitektur:} Die angepassten Netzarchitekturen sollten in diesem Szenario keine signifikante Leistungssteigerung gegenüber der Standard-Gradient\hyp{}Gating-Variante zeigen, da alle Aktionen etwa gleich häufig auftreten und die Interferenz zwischen den Aktionszweigen gering sein sollten. 
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item \textbf{PPO mit Gating:} Der Standard-Algorithmus in der modifizierten Umgebung, erweitert um den in Kapitel \ref{ch:ppo_bedingt} beschriebenen Gradient\hyp{}Gating-Mechanismus.
    \end{itemize}
...
\begin{figure}[htbp]
    ...
    \caption[Finale Trainingsleistung (Test 1)]{Finale Trainingsleistung (Last Eval)}
...
\begin{figure}[htbp]
    ...
    \caption[Trainingsleistung Mean (Test 1)]{Durchschnittliche Trainingsleistung (Mean Eval)}
...
\begin{figure}[H]
    ...
    \caption[Performance Profile (Test 1)]{Performance Profile der finalen Leistung}
...
\begin{itemize}
    ...
    \item \textbf{Masked PPO:} Der um Gradient\hyp{}Gating erweiterte PPO-Algorithmus.
...
Der PPO-Algorithmus erzielt zwar den höchsten durchschnittlichen Trainingswert (IQM 224.84), bricht jedoch gegen Ende leistungstechnisch ein (Last Eval 202.21). Dies indiziert eine \emph{Policy Degradation}, also das Verlernen bereits erworbenen Wissens.
...
Um die Divergenz zwischen der Instabilität des PPO und der Robustheit der Gating-Varianten zu erklären, ist eine Analyse der zugrunde liegenden Verhaltensstrategien notwendig. Diese zeigt, dass Gradient\hyp{}Gating das Lernverhalten des Agenten fundamental verändert.
...
\begin{figure}[htbp]
    ...
    \caption[Noop-Aktionen (Test 2)]{Entwicklung der durchschnittlichen Noop-Aktionen (geglätteter Mittelwert über alle Läufe). Der PPO-Algorithmus (Grau) zeigt eine überproportionale Tendenz zur Inaktivität.}
...
\begin{figure}[htbp]
    ...
    \caption[Clip Fraction (Test 2)]{Clip Fraction im Trainingsverlauf (geglätteter Mittelwert über alle Läufe). Die hohen Werte des PPO-Algorithmus (Grau) indizieren instabile Policy-Updates.}
...
\begin{figure}[htbp]
    ...
    \caption[Entropy Loss (Test 2)]{Entwicklung des Entropy Loss (geglätteter Mittelwert über alle Läufe). Vergleich Gating-Varianten (Pink, Rot, Orange) und PPO-Algorithmus (Grau)}
